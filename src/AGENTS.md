...
- Light.PortableResults contains the core implementation which is framework-agnostic. It uses .NET
...
- Light.PortableResults.Validation contains the framework-agnostic validation functionality for DTO-centric workflows.
  It targets .NET Standard 2.0, too, builds on top of the core result/error primitives, and is intentionally optimized
  for low-allocation validation flows. This project is compatible with Native AOT.
...
- Light.PortableResults.AspNetCore.Mvc contains specific types for MVC integration. `<IsAotCompatible>` is not set
  because MVC itself is not AOT-compatible.
