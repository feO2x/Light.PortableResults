...
- **Digits:** the shortest decimal digit sequence that parses to the same binary value under round-to-nearest-even. If several sequences of that length round-trip, choose the one nearest the exact value; an exact tie resolves to an even final digit.
...
- **Notation:** with the value represented as `0.d₁d₂… × 10^s`, use positional notation when `-3 <= s <= 17` for `double` and `-3 <= s <= 9` for `float`; otherwise use scientific notation with uppercase `E`, an explicit exponent sign, and at least two exponent digits.
