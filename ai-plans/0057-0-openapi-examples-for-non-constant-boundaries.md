...
- [ ] `README.md` no longer states that examples require compile-time constant metadata arguments, and describes what is reconstructed and what degrades.
- [ ] Test code coverage stays above 95%, and no existing generated output changes for boundaries that already fold today.
...
| --- | --- |
| `DateTime` | `(int, int, int)`, `(int, int, int, int, int, int)`, `(int, int, int, int, int, int, int)`, each also with a trailing `DateTimeKind`; `(long)`; `(long, DateTimeKind)` |
...
| `default`, `default(T)`, target-typed `new()` | Keeps the recognizer to two syntax forms, object creation and member access or invocation |
