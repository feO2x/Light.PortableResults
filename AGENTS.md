...
- `Movie.Ratings` had to gain an `init` setter for the MongoDB driver. `BsonClassMap` needs a settable member to populate the embedded ratings, and it also rejects explicit member mappings for properties declared on a base class, which is why the class maps use `AutoMap` - `Id` comes from `GuidEntity`. EF Core is unaffected because it maps the backing field.
